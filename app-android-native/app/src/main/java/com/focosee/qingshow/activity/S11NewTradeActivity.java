...
public class S11NewTradeActivity extends BaseActivity {
  ...
  TextView hint;
  @InjectView(R.id.page_left)
  TextView left;
  @InjectView(R.id.page_right)
  TextView right;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    submit.setText(StringUtil.FormatPrice(
        itemEntity.promoPrice.floatValue() - itemEntity.expectable.reduction.floatValue())
        + " 购买");
  ...
  private OnCheckedChangeListener onCheckedChangeListener = new OnCheckedChangeListener() {
    ...
    public void onChanged(String key, int index) {
      ...
      if (SkuHelper.obtainSkuStock(itemEntity.skuTable,
          SkuUtil.formetPropsAsTableKey(selectProps, keys_order)) < 1) {
        changeBtnClickable(false);
      } else {
    ...
    for (String p : keys_order) {
      ...
      for (String value : props.get(p)) {
        ...
        if (tempMap.keySet().size() == props.size()) {
          isAble = SkuHelper.obtainSkuStock(itemEntity.skuTable,
              SkuUtil.formetPropsAsTableKey(tempMap, keys_order)) >= 1;
          if (isAble) continue;
  ...
  private void submitToNet(MongoTrade trade) {
    ...
    try {
      params.put("selectedSkuProperties",
          new JSONArray(QSGsonFactory.create().toJson(trade.selectedSkuProperties)));
    ...
    QSJsonObjectRequest jsonObjectRequest =
        new QSJsonObjectRequest(Request.Method.POST, QSAppWebAPI.getTradeCreateApi(),
        ...
        .subscribe(new QSSubscriber<List<MongoPeople>>() {
          ...
          public void onNext(List<MongoPeople> mongoPeoples) {
            if (mongoPeoples != null && mongoPeoples.size() > 0) {
              buyers.setLayoutManager(
                  new LinearLayoutManager(S11NewTradeActivity.this, LinearLayoutManager.HORIZONTAL,
                      false));
              buyers.setAdapter(new TopOwnerAdapter(mongoPeoples, S11NewTradeActivity.this,
  ...
  private void addItemToCanvas(QSCanvasView canvas, MongoItem item, final RectF rectF) {
    ...
    ImageLoader.getInstance()
        .displayImage(item.thumbnail, imageView.getImageView(), AppUtil.getShowDisplayOptions(),
  ...
  private void bindItem(final String key, final List<String> values, final int position,
      final OnCheckedChangeListener onCheckedChangeListener) {

    LinearLayout prop =
        (LinearLayout) this.getLayoutInflater().inflate(R.layout.item_sku_prop, null);
    ...
    ViewGroup.MarginLayoutParams itemParams =
